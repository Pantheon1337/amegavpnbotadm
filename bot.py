...
def main():
    try:
        ...
        if application.job_queue:
            application.job_queue.run_daily(send_payment_reminder, time=time(hour=12, minute=0))
        else:
            logger.warning("JobQueue не доступен. Напоминания об оплате не будут отправляться.")
